...
class ImagesMockServerSpec extends BaseMockServerSpec {
  ...
  "images" should {
    ...
    "succesfully get image revisions" in {
      val response = resourceAsString("/org/scalawiki/query/Commons_Wiki_Loves_Earth_2019_Winners_revisions.json")
      val action = Map(
        "action" -> "query",
      ...
      val future = bot.page("Commons:Wiki_Loves_Earth_2019/Winners")
        .revisionsByGenerator("images", "im",
          props = Set("ids", "content", "user", "comment"), limit = "50")
      val info = future.await
      info should not(beEmpty)
      info.size === 50
      info.map(_.revisions.size) === List.fill(50)(1)
      info.map(_.revisions.head.content.exists(_.nonEmpty)) === List.fill(50)(true)
    }
  }
}
