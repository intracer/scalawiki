...
case class Title(override val arg: String)
    ...
    with EditParam[String]

case class PageId(override val arg: Long)
    extends IdParameter(
      "pageid",
...
case class Section(override val arg: String)
    extends StringParameter(
      "section",
...
case class SectionTitle(override val arg: String)
    extends StringParameter(
      "sectiontitle",
      "Title to use if creating a new section. If not specified, summary will be used instead"
    )
    with EditParam[String]

case class Text(override val arg: String)
    extends StringParameter("text", "New page (or section) content")
    with EditParam[String]

case class Token(override val arg: String)
    extends StringParameter("token", "Edit token")
    with EditParam[String]

case class Summary(override val arg: String)
    extends StringParameter(
      "summary",
...
case class Minor(override val arg: Boolean = true)
    extends BooleanParameter("minor", "If set, mark the edit as minor")
    with EditParam[Boolean]

case class NotMinor(override val arg: Boolean = true)
...
case class Bot(override val arg: Boolean = true)
    extends BooleanParameter(
      "bot",
...
case class BaseTimestamp(override val arg: ZonedDateTime)
...
case class StartTimestamp(override val arg: ZonedDateTime)
    extends DateTimeParameter(
      ...
      "Timestamp when you started editing the page (e.g., when you fetched the current revision's text to begin editing it or checked the (non-)existence of the page). " +
        "Used to detect if the page has been deleted since you started editing; leave unset to ignore conflicts"
    )
    with EditParam[ZonedDateTime]

case class Recreate(override val arg: Boolean = true)
    extends BooleanParameter(
      "recreate",
...
case class CreateOnly(override val arg: Boolean = true)
    extends BooleanParameter(
      "createonly",
...
case class NoCreate(override val arg: Boolean = true)
    extends BooleanParameter(
      "nocreate",
...
case class Md5(override val arg: String)
    extends StringParameter(
      "md5",
      "MD5 hash (hex) of the text parameter or the prependtext and appendtext parameters concatenated. " +
        "If this parameter is set and the hashes don't match, the edit is rejected."
    )
    with EditParam[String]

case class PrependText(override val arg: String)
    extends StringParameter(
      "prependtext",
...
case class AppendText(override val arg: String)
    extends StringParameter(
      "appendtext",
...
case class Undo(override val arg: Long)
    extends IdParameter(
      "appendtext",
      "Revision ID to undo. Overrides text, prependtext and appendtext"
    )
    with EditParam[Long]

case class UndoAfter(override val arg: Long)
    extends IdParameter(
      "undoafter",
      "Undo all revisions from undo up to but not including this one. If not set, just undo one revision"
    )
    with EditParam[Long]

case class Redirect(override val arg: Boolean = true)
    extends BooleanParameter("redirect", "Automatically resolve redirects")
    with EditParam[Boolean]
