...
case class ListParam(override val args: ListArg*)
    extends EnumParameter[ListArg]("list", "")
    with QueryParam[ListArg]

/** ?action=query&amp;list=argument
  */
trait ListArg extends EnumArg[ListArg] with GeneratorArg {
  val param = ListParam
}
