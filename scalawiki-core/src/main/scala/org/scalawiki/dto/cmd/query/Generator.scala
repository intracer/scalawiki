...
case class Generator(generator: EnumArg[GeneratorArg])
    extends /*ArgWithParams[G, ListArg] with */ QueryParam[
      EnumArg[GeneratorArg]
    ] {
  ...
  override def pairs: Seq[(String, String)] =
    Seq(name -> generator.name) ++ generator.pairs.map { case (k, v) =>
      ("g" + k, v)
    }
}
