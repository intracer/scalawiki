...
case class Watch(override val arg: Boolean = true)
    extends BooleanParameter(
      "watch",
      "Add the page to your watchlist. Deprecated. Use the watchlist argument"
    )
    with WatchParam[Boolean]

case class UnWatch(override val arg: Boolean = true)
    extends BooleanParameter(
      "unwatch",
...
case class WatchList(override val args: WatchListArg*)
    extends EnumParameter[WatchListArg](
      "watchlist",
      "Specify how the watchlist is affected by this edit"
    )
    with WatchParam[WatchListArg]

trait WatchListArg extends EnumArg[WatchListArg] { val param = WatchList }

object WLWatch
    extends EnumArgument[WatchListArg](
      "watch",
