...
class SkippingInputStreamSpec extends Specification {
  ...
  "SkippingInputStream" should {
    ...
    "read nothing" in {
      ...
      val sis = new SkippingInputStream(
    ...
    "read first two blocks to large buf" in {
      val input =
        ("1234567890" + "qwertyuiop" + "asdfghjkl" + "zxcvbnm").getBytes
      val sis = new SkippingInputStream(
        new ByteArrayInputStream(input),
        Seq(Block(0, 10), Block(10, 10))
      )
