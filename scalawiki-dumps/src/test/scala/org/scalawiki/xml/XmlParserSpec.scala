...
class XmlParserSpec extends Specification {
  ...
  "parse a page with entities" in {
    ...
    val (revId, parentId, timestamp, user, userId, comment, text, sha1) =
      (
        ...
        567,
        "revision comment",
        "<span style=\"display:none\">Main page</span>",
        "sha1"
      )

    val revsXml =
      revisionXml(revId, parentId, timestamp, user, userId, comment, text, sha1)
  ...
  def checkRevision(
    ...
    (
      revision.id,
      revision.parentId,
      revision.timestamp.map(Timestamp.format),
      revision.comment,
      revision.content
    ) ===
