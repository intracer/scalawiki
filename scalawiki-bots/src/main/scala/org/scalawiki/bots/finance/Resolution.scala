...
case class Resolution(
  ...
  def substRealNames(text: String): String = {
    var replaced = text
    Resolution.realNames.foreach { case (nick, name) =>
  ...
  def htmlText: String = {
    try {
      ...
      val noEndingDate =
        noLinks.replaceAll("\\d+\\.\\d{2}\\.\\d{4}, м. Київ\\.?", "")

      val voteR = "''\\(«За»[^\\)]+\\)''".r
      val withVotes = voteR.replaceAllIn(
        noEndingDate,
        m => {
...
object Resolution {
  ...
  def fromPage(page: Page): Option[Resolution] = {
    ...
    val regex =
      "Рішення Правління №(\\d+)\\/(\\d{4}) від (\\d+) (\\p{L}+) (\\d{4})"
        .r("num", "year1", "day", "month", "year2")

    regex.findFirstMatchIn(title).map { m =>
