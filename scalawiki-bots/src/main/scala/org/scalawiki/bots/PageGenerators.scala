...
case class PageGenConfig(
    cat: Seq[String] = Seq.empty, // ,
    //                         catR: String,
    //                         subCats: String,
    //                         subCatsR: String,
    //                         unCat: String,
...
class PageGenerators(arguments: Seq[String]) extends ScallopConf(arguments) {
  val category = opt[String](
    "cat",
    descr = "Work on all pages which are in a specific category.",
    required = false
  )
  val namespace = opt[String](
    "namespace",
    descr = "Work on all pages in given namespaces.",
    required = false
  )
}
