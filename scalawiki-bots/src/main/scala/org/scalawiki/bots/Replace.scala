...
class Replace(arguments: Seq[String]) extends PageGenerators(arguments) {
  val regex = opt[Boolean](
    "regex",
    descr = "Make replacements using regular expressions.",
    default = Some(false)
  )
  val replacements = trailArg[List[String]](
    descr = "Replacement pairs.",
    required = false,
    default = Some(Nil)
  )
  verify()
...
object TextLib {
  ...
  /** @param text
    *   text to process
    * @param old
    *   regex pattern string to replace
    * @param replacement
    *   replacement
    * @param exceptions
    *   text that matches these regex exceptions should not have replacements
    * @param ignoreCase
    *   match both lower- and uppercase characters for replacement
    * @param marker
    *   add this marker after the last replace or the end of text
    * @return
  ...
  def replaceExcept(
    ...
    val regex =
      new ReplacementBase(old, replacement, None, true, ignoreCase).regex
    var markerPos = text.length
    val exceptionMatches =
      exceptions.map(ex => ex.findAllMatchIn(text).buffered)
