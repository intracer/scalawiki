...
class RevisionStat(
  ...
  val usersSorted = byUserSize.toSeq.sortBy { case (user, size) =>
    -size
  }

  val usersSortedString = usersSorted
    .map { case (user, size) =>
      s"[[User:$user|$user]]: $size"
    }
    .mkString("<br>")

  def stat =
    s"| [[${page.title}]] || $addedOrRewritten || $delta || ${byUserSize.keySet.size} || $usersSortedString"
...
object RevisionStat {
  ...
  def statHeader =
    s"! title !! added or rewritten !! size increase !! users !! users by added or rewritten text size\n"

  def fromPage(page: Page, revFilter: RevisionFilter = AllRevisionsFilter) = {
    val annotation = new RevisionAnnotation(page, revFilter)

    val byRevisionSize = annotation.byRevisionContent.map { case (rev, words) =>
      rev.withoutContent -> words
        .map(_.getBytes(StandardCharsets.UTF_8).length.toLong)
        .sum
    }

    val byUserSize = annotation.byUserContent
      .mapValues(_.map(_.getBytes(StandardCharsets.UTF_8).length.toLong).sum)
      .toMap
