...
class DslQueryDbCacheBlackBoxSpec
  ...
  "get revisions text with generator and caching" should {
      ...
      val expectedCommands = Seq(
        HttpStub(
          Map(
            ...
            "gcmlimit" -> "max",
            "prop" -> "info|revisions",
            "rvprop" -> "ids|user|userid", // TODO remove user?
            "continue" -> ""
          ),
          responseWithRevIds()
        ),
    ...
    "from cache in" in {
      ...
      byRevs.map(_.revisions.head.content.get) === Seq(
        "some vandalism",
        "more vandalism"
      )
    ...
    "add page to cache" in {
      val expectedCommands = Seq(
        ...
        HttpStub(
          ...
          pagesJson(Seq(page1(Some(pageText1))))
        ),
      ...
      pageDao.list.size must be_==(2).eventually
      val byRevsFinal =
        pageDao.findByRevIds(Seq(4571809L, 569559L), Seq(12, 11))
      byRevsFinal.size === 2
      byRevsFinal.map(_.title) === Seq("Talk:Welfare reform", "User:Formator")
      byRevsFinal.map(_.revisions.head.content.get) === Seq(
        "some vandalism",
        "more vandalism"
      )
    ...
    "add one page revision to cache" in {
      ...
      val expectedCommands = Seq(
        ...
        HttpStub(
          ...
          pagesJson(Seq(page1(Some(pageText1), revId = 11)))
        ),
        HttpStub(
          Map(
            "action" -> "query",
        ...
        HttpStub(
          ...
          pagesJson(Seq(page1(Some(pageText2), revId = 12)))
        ),
        HttpStub(
          Map(
            "action" -> "query",
