...
1. **Monument lists**: `MonumentQuery` fetches/parses monument list pages from wiki (via `WlxTemplateParser`/`WlxTableParser`) into `Monument` DTOs; `MonumentDB` indexes them by id/region/type for a given `Contest`.
   - **Monument CSV cache** (on by default, same `--no-csv-cache` toggle): `MonumentDbCache` serializes parsed monuments to `csv-cache/<campaign>-monuments.csv`, each row tagged with its source page + that page's `page_revid`/`page_ts` (`otherParams` as a JSON column, `listConfig` re-attached on read). `Statistics.gatherMonumentDb` drives it: with a cache present it runs a cheap `MonumentQuery.listPageRevs` (`embeddedin` id+timestamp sweep, no content), refetches only pages whose revid changed via `MonumentQuery.monumentsByPages`, drops pages a complete sweep confirms are gone (a short sweep never mass-deletes), reuses the rest, rewrites the cache; any sync error falls back to a full fetch. `--monument-cache-refresh` (or `--csv-cache-refresh`) forces the full path.
