 ...
 * @version 0.28
 */
public class Wiki implements Serializable, IWiki {
    // Master TODO list:
    // *Admin stuff (low priority)
    // *More multiqueries
    // *Generators (hard) - fucking easy
    ...
    public String getPageText(String title) throws IOException {
        ...
        // go for it
        String url = base + getEncode(title) + "&action=raw";
        String temp = fetch(url, "getPageText");
        log(Level.INFO, "Successfully retrieved text of " + title, "getPageText");
        return decode(temp);
    }

    private String getEncode(String title) throws UnsupportedEncodingException {
        if (true) {
            return URLEncoder.encode(normalize(title), "UTF-8");
        }  else {
            return title;
        }
    }
    ...
    protected String evaluateXpath(String s, String xpath) {
        ...
        try {
            ...
            InputSource content = new InputSource(byteStream);
            return titleParser.evaluate(content);
    ...
    @Override
    public List<Revision> getPagesText(String... titles) throws IOException {
        StringBuilder url = new StringBuilder(query);
        ...
        String text = fetch(url.toString(), "getPagesText");

        // if the section does not contain any text, <rev xml:space=\"preserve\">
        // will not have a separate closing tag

        List revs = new ArrayList<Revision>();
        for (String title : titles) {
            String xpath = "//page[@title=\"" + title + "\"]/revisions/rev/text()";
            String content = evaluateXpath(text, xpath);
            String decoded = decode(content);
            Revision rev = new Revision(title, decoded);
            revs.add(rev);
        }
    ...
    public String[] getImagesOnPage(String title) throws IOException {
        String url = query + "prop=images&imlimit=max&titles=" + getEncode(title);
    ...
    public HashMap<String, String> getInterWikiLinks(String title) throws IOException {
        String url = query + "prop=langlinks&lllimit=max&titles=" + getEncode(title);
    ...
    public String export(String title) throws IOException {
        return fetch(query + "export&exportnowrap&titles=" + getEncode(title), "export");
    }
    ...
    protected void watchInternal(boolean unwatch, String... titles) throws IOException, CredentialNotFoundException {
        ...
        for (int i = 0; i < titles.length; i++) {
            ...
            data.append(getEncode(titles[i]));
            if (unwatch)
                data.append("&unwatch");
            data.append("&token=");
     ...
     */
    @Override
    public Page[] whatTranscludesHere(String title, int... ns) throws IOException {
        StringBuilder url = new StringBuilder(query);
        ...
        // main loop
        ArrayList<Page> pages = new ArrayList<Page>(6667); // generally enough
        String next = "";
        do {
            // fetch data
    ...
    protected LogEntry parseLogEntry(String xml) {
        ...
        if (xml.contains("title=\"")) {
            String title = parseAttribute(xml, "title", 0);
            if (title != null) {
                target = decode(title);
            }
        }
    ...
    public class User implements Cloneable {
        ...
        public HashMap<String, Object> getUserInfo(boolean skip) throws IOException {

            String props = skip
                    ? "editcount%7Cemailable%7Cblockinfo%7Cgender%7Cregistration"
                    : "editcount%7Cgroups%7Crights%7Cemailable%7Cblockinfo%7Cgender%7Cregistration";
            String info = fetch(query + "list=users&usprop=" + props + "&ususers="
            ...
            ret.put("created", parseAttribute(info, "registration", 0));
    ...
    public class Revision implements Comparable<Revision> {
        ...
        public Revision(String title, String content) {
            this.title = title;
    ...
    public String getSulEditCount(String user) throws IOException {
        String url = query + "meta=globaluserinfo&guiuser=" + URLEncoder.encode(user, "UTF-8") + "&guiprop=editcount";
        String response = fetch(url, "getSulEditCount");
        String paramName = "editcount=\"";
        int paramStart = response.indexOf(paramName);
        int start = paramStart + paramName.length();
        int end = response.indexOf("\"", start);
        if (paramStart > 0 && end > start) {
