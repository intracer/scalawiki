...
class PageQuery(query: Either[Set[Int], Set[String]]) {
...
class SinglePageQuery(query: Either[Int, String]) extends PageQuery(query.fold(id => Left(Set(id)), title => Right(Set(title))))
...
object PageQuery {
  ...
  def byIds(ids: Set[Int]) = new PageQuery(Left(ids))

  def byId(id: Int) = new SinglePageQuery(Left(id))
}
