...
class Template1(
                 val text: String,
                 val containingPage: String = "",
                 names: Map[String, String] /*= Map.empty*/) extends Template {
  ...
  def getParam(name: String, withoutComments: Boolean = true): String = {
    ...
    if (withoutComments)
      Template1.removeComments(param).trim
    else param.trim
  }

  def getParamOpt(name: String): Option[String] = findPosition(name)
  ...
  def setTemplateParam(name: String, value: String): Template = findPosition(name).fold[Template](this) {
    case (start, end) =>
      init(text.substring(0, start) + value + text.substring(end), containingPage, names)
  }

  private def matcher(text: String, param: String): Matcher = {
    val mappedParam = names.getOrElse(param, param)
    val p = Pattern.compile("\\|\\s*" + mappedParam + "\\s*=")
    p.matcher(text)
  }

  private def findPosition(param: String): Option[(Int, Int)] = {
  ...
  def init(text: String, page: String, names: Map[String, String]): Template = new Template1(text, page, names)
