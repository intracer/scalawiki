 ...
 * Every edit of a page creates a revision row, which holds information such as the user who made the edit,
...
class Revisions(tag: Tag) extends Table[Revision](tag, "revision") {
   ...
   * It's possible for multiple revisions to use the same text—for instance, revisions where only metadata is altered,
   ...
   * (The recentchanges table contains a copy used for recent changes, related changes, watchlists, and, in the case of page creation,
   ...
   * In anonymous revisions imported from UseModWiki or early incarnations of the Phase II software,
   * this field may contain an IP address with the final octet obscured (i.e. \d{1,3}\.\d{1,3}\.\d{1,3}\.xxx such as 24.150.61.xxx;
   ...
   * If the value for this field is 1, then the edit was declared as 'minor';
   ...
   * So, for example, if nothing has been deleted from that revision, then the value is 0;
