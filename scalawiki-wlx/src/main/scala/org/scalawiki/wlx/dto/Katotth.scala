...
object Katotth {
  ...
  lazy val toKatotth: Map[String, Seq[String]] =
    toKoatuu.toSeq.groupBy(_._2).mapValues(_.map(_._1)).toMap

  def toFlat(row: Seq[String]): AdmDivisionFlat = {
    val mainLevels =
      row.take(4).filterNot(_.isEmpty).map(_.replace("UA", "")).distinct
    val codes = mainLevels.zipWithIndex.map { case (c, level) =>
      shortCode(c, level + 2)
    }
    val additional = Seq(row(4))
      .filterNot(_.isEmpty)
      .map(mainLevels.last + _.replace("UA", ""))
    AdmDivisionFlat(
      codeLevels = codes ++ additional,
  ...
  def readCsv(filename: String): Seq[List[String]] = {
    val csv = CSVReader.open(
      new InputStreamReader(
        getClass.getResourceAsStream(s"/$filename.csv"),
        "UTF-8"
      )
    )
    csv.all().tail
