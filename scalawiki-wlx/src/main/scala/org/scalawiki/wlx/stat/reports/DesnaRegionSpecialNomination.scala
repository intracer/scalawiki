...
class DesnaRegionSpecialNomination(data: Seq[List[String]]) {
  ...
  val places: Seq[List[String]] =
    data.map(_.filter(_.trim.nonEmpty)).filter(_.size == 4)
  ...
  def getRaion(
    ...
    (for (
      oblast <- getOblast(
        row,
        country: Country,
        suffixes.headOption.getOrElse("")
      )
    )
      yield getRaion(
        oblast,
        row(1) + suffixes.lastOption.getOrElse(""),
        country: Country
      )).getOrElse(Nil)
  }
  ...
  def getRaion(
    ...
    candidates.filter(
      _.regionType.exists(_.code == "P") || name.endsWith(" район")
    )
  }
