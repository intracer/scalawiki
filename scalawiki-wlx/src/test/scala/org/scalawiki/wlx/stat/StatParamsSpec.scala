  ...
  "statistics" should {
    ...
    "parse new object rating" in {
      StatParams.parse(Seq("--campaign", "wle-ua", "--new-object-rating", "7")) ===
        StatConfig("wle-ua", Seq(thisYear), rateConfig = RateConfig(newObjectRating = Some(7)))

      StatParams.parse(Seq("--campaign", "wle-ua", "--new-author-object-rating", "3")) ===
        StatConfig("wle-ua", Seq(thisYear), rateConfig = RateConfig(newAuthorObjectRating = Some(3)))

      StatParams.parse(Seq("--campaign", "wle-ua", "--new-object-rating", "10", "--new-author-object-rating", "5")) ===
        StatConfig("wle-ua", Seq(thisYear), rateConfig = RateConfig(newObjectRating = Some(10), newAuthorObjectRating = Some(5)))
    }

    "parse bonus" in {
      StatParams.parse(Seq("--campaign", "wle-ua", "--number-of-authors-bonus")) ===
        StatConfig("wle-ua", Seq(thisYear), rateConfig = RateConfig(numberOfAuthorsBonus = true))

      StatParams.parse(Seq("--campaign", "wle-ua", "--number-of-images-bonus")) ===
        StatConfig("wle-ua", Seq(thisYear), rateConfig = RateConfig(numberOfImagesBonus = true))

      StatParams.parse(Seq("--campaign", "wle-ua", "--number-of-authors-bonus", "--number-of-images-bonus")) ===
        StatConfig("wle-ua", Seq(thisYear), rateConfig = RateConfig(numberOfAuthorsBonus = true, numberOfImagesBonus = true))
    }

    "parse gallery" in {
      StatParams.parse(Seq("--campaign", "wle-ua")) === StatConfig("wle-ua", Seq(thisYear))
      StatParams.parse(Seq("--campaign", "wle-ua", "--gallery")) === StatConfig("wle-ua", Seq(thisYear), gallery = true)
    }

    "parse place-detection" in {
      StatParams.parse(Seq("--campaign", "wle-ua")) === StatConfig("wle-ua", Seq(thisYear))
      StatParams.parse(Seq("--campaign", "wle-ua", "--place-detection")) === StatConfig("wle-ua", Seq(thisYear), placeDetection = true)
    }
